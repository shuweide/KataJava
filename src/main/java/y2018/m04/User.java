...
public class User {
    ...
    public void incProgress(int kataRank) {
        if (Math.abs(kataRank) > 8 || kataRank == 0) {
            throw new IllegalArgumentException("Invalid rank");
        }
        int dRank = kataRank - rank + (rank > 0 ? 1 : -1) * (kataRank * rank < 0 ? 1 : 0);
        int point = dRank > 0 ? 10 * dRank * dRank : dRank == 0 ? 3 : dRank > -2 ? 1 : 0;
        progress += point;
        int upLevel = progress / 100;
        rank += upLevel + (rank * (rank + upLevel) <= 0 ? 1 : 0);
        progress = rank >= MAX_RANK ? 0 : (progress) % MAX_PROGRESS;
    }
}

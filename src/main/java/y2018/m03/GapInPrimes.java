...
public class GapInPrimes {
    /**
     * ```java
     * long[] primes = LongStream.rangeClosed(start, end)
     *      .filter(GapInPrimes::isPrime).toArray();
     * long[] result = null;
     *
     * for (int i = 1; i < primes.length; i++) {
     *      if (primes[i] - primes[i - 1] == findGap) {
     *          result = new long[]{primes[i - 1], primes[i]};
     *          break;
     *      }
     * }
     * return result;
     ...
     * @return
     */
    public static long[] gap(int findGap, long start, long end) {
        long[] primes = LongStream.rangeClosed(start, end)
                .filter(GapInPrimes::isPrime).toArray();
        long[] result = null;
    ...
    /**
     * ```java
     * return LongStream.iterate(start % 2 == 0 ? start + 1 : start, l -> l + 2).limit((end - start) / 2)
     *      .filter(l -> BigInteger.valueOf(l).isProbablePrime(5) && BigInteger.valueOf(l + findGap).isProbablePrime(5))
     *      .filter(l -> LongStream
     *          .iterate(l + 2, c -> c + 2)
     *          .limit((findGap - 2) / 2).parallel()
     *          .filter(c -> BigInteger.valueOf(c).isProbablePrime(5))
     *          .mapToObj(c -> false)
     *          .findAny().orElse(true)
     *      )
     *      .mapToObj(l -> new long[]{l, l + findGap})
     *      .findFirst()
     *      .orElse(null);
     * ```
     * @param findGap
     ...
     * @return
     */
    public static long[] gapBest(long findGap, long start, long end) {
        return LongStream.iterate(start % 2 == 0 ? start + 1 : start, l -> l + 2).limit((end - start) / 2)
                .filter(l -> BigInteger.valueOf(l).isProbablePrime(5) && BigInteger.valueOf(l + findGap).isProbablePrime(5))
                .filter(l -> LongStream
                        .iterate(l + 2, c -> c + 2)
     ...
     * return LongStream.rangeClosed(2, candidateRoot).noneMatch(i -> candidate % i == 0);
     * ```
     * @param candidate
     * @return
     */
